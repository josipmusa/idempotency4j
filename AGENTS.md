...
- **Engine** (`idempotency-core`): transport- and framework-agnostic orchestrator - no HTTP types live here, so non-HTTP callers drive it directly. Calls `tryAcquire`, runs the action with a lease-extending heartbeat (fires at leaseDuration/2), calls `release` on any `Throwable` from the action. Does not decide *when* a key is complete - the adapter calls `engine.complete(...)` once it knows what to store, and the engine forwards that to `store.complete` and fires the lifecycle callbacks around it.
- **Adapter** (`spring/idempotency-spring-web`): `IdempotencyFilter` builds the context (scope from `IdempotentHandlerRegistry`, key from the header), invokes the engine, captures the HTTP response, and is the one that decides completion - through `engine.complete()` with the engine-provided lease, never `store.complete()` directly. Servlet translation (capture, replay, error bodies) lives in `HttpIdempotencyMapper`; HTTP-only settings live in `WebIdempotencyConfig` and `ResponseSanitizer`. Maps exceptions to HTTP: in-flight past the wait timeout -> 503, fingerprint mismatch -> 422.
