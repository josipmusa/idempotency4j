...
For Redis deployments, use TLS for remote connections, authenticate with an ACL restricted to the configured key prefix, and use a dedicated logical namespace. Configure `maxmemory-policy noeviction`: eviction of a live idempotency record can permit the protected action to execute again. Enable persistence appropriate to the required recovery objective and remember that Sentinel failover uses asynchronous replication; Redis `WAIT` can reduce but cannot eliminate acknowledged-write loss.
